...
print(counter)      #4
print('--------------------------字符串相关的方法/函数----------------------------------------------')
#upper()方法：字符串中小写的英文字母转换为大写的英文字母，返回值为字符串
#lower()方法：字符串里大写的英文字母转换为小写的英文字母，返回值为字符串
#strip()方法：字符串的开头或结尾移除指定的字符（如：字母、数字、空格、换行符\n、标点符号等），默认移除开头、结尾所有空格和换行符
#count()方法：判断字符串给定的字母/数字的个数，返回值为整数
#len()方法：判断字符串的长度，返回值为整数
#split()方法：将字符串转换为列表
Comany_ip='172.168.2.200'
print(type(Comany_ip.split('.')),Comany_ip.split('.'))     #将IP地址剥离为4个元素的列表，用.隔开
#join()方法：将列表转换为字符串
School_ip=['192','168','3','250']
print(type('.'.join(School_ip)) ,'.'.join(School_ip))      #打印转换后的类型和结果
